...
class _$BaseStateTearOff {
  ...
  Loaded<T> loaded<T>(T data) {
    return Loaded<T>(
      data,
    );
  }

  Error<T> error<T>(dynamic error) {
    return Error<T>(
      error,
    );
  }
...
/// @nodoc
class _$EmptyCopyWithImpl<T, $Res> extends _$BaseStateCopyWithImpl<T, $Res>
...
abstract class Empty<T> implements BaseState<T> {
...
abstract class Loaded<T> implements BaseState<T> {
  const factory Loaded(T data) = _$Loaded<T>;
...
abstract class Error<T> implements BaseState<T> {
  const factory Error(dynamic error) = _$Error<T>;
