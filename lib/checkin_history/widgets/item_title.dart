...
class ItemTitle extends StatelessWidget {
  ...
  FaceMaskStatus _getFaceMaskStatus(int status) {
    switch (status) {
      case 100:
        return FaceMaskStatus(status: 100, title: 'Không đeo khẩu trang', color: Util.errorColor);
      case 200:
        return FaceMaskStatus(status: 200, title: 'Khẩu trang hợp lệ', color: Util.successColor);
      case 210:
        return FaceMaskStatus(status: 210, title: 'Không đeo khẩu trang', color: Util.warningColor);
      default:
        return FaceMaskStatus(status: 000, title: 'Chưa có thông tin', color: Util.warningColor);
    }
  }
  ...
  TemperatureStatus _getTemperatureStatus(int status) {
    switch (status) {
      case 100:
        return TemperatureStatus(status: 100, title: 'Nhiệt độ bình thường', color: Util.successColor);
      case 200:
        return TemperatureStatus(status: 200, title: 'Nhiệt độ cao', color: Util.errorColor);
      default:
        return TemperatureStatus(status: 0, title: 'unknow', color: Util.errorColor);
    }
  }
}
